...
async def test_runner_passes_absolute_workdir_to_codex(tmp_path, monkeypatch):
    fixture = tmp_path / "fake_codex.py"
    fixture.write_text(
    ...
    workdir = tmp_path / "relative-run"
    workdir.mkdir()
    monkeypatch.chdir(tmp_path)
    runner = CodexCliRunner(command=[sys.executable, str(fixture)])

    await runner.run("x", Path("relative-run"), lambda *args: None)

    args = json.loads((workdir / "argv.json").read_text(encoding="utf-8"))
    workdir_argument = args[args.index("-C") + 1]
    assert workdir_argument == str(workdir.resolve())


async def test_runner_configures_headed_chrome_in_an_offscreen_window(tmp_path):
    fixture = tmp_path / "fake_codex.py"
    fixture.write_text("pass\n", encoding="utf-8")
    runner = CodexCliRunner(command=[sys.executable, str(fixture)])

    await runner.run("x", tmp_path, lambda *args: None)

    config_path = tmp_path / ".playwright-mcp-config.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    browser = config["browser"]
    assert browser["isolated"] is True
    assert browser["launchOptions"]["headless"] is False
    assert browser["launchOptions"]["channel"] == "chrome"
    assert "--window-position=-10000,-10000" in browser["launchOptions"]["args"]
    assert "--disable-backgrounding-occluded-windows" in browser["launchOptions"]["args"]
    assert browser["contextOptions"]["viewport"] == {"width": 1280, "height": 800}

    command = runner.build_command(tmp_path.resolve())
    playwright_args = next(
        value for value in command if value.startswith("mcp_servers.playwright.args=")
    )
    assert "--config" in playwright_args
    assert str(config_path.resolve()) in playwright_args
    assert "--isolated" not in playwright_args


async def test_runner_accepts_large_single_line_json_events(tmp_path):
    fixture = tmp_path / "large_event.py"
    fixture.write_text(
        "import json\nprint(json.dumps({'type': 'item.completed', 'text': 'x' * 100_000}))\n",
        encoding="utf-8",
    )
    captured = []
    ...
    assert captured[0][1] == "item.completed"
