...
class DeepSeekChatProvider:
    ...
    async def generate(self, request: NodeLLMRequest) -> LLMCallResult:
        ...
        try:
            ...
            response.raise_for_status(); data = response.json(); choice = data.get("choices", [{}])[0]; raw = choice.get("message", {}).get("content", "")
            usage = data.get("usage", {})
            return LLMCallResult(raw, _json_text(raw), data.get("id"), "deepseek", self.model, usage.get("prompt_tokens"), usage.get("completion_tokens"), usage.get("total_tokens"), round((time.monotonic()-started)*1000), choice.get("finish_reason"))
        finally:
            if own: await client.aclose()
