...
def parse_run_result(workdir: Path) -> ParsedRunResult:
    ...
    final_state = _clean(payload.get("final_state") or payload.get("status"))
    stop_node = _clean(payload.get("stop_node"))
    if final_state != "WAITING_HUMAN_EVALUATION" or stop_node != "H02":
        stop_reason = _clean(payload.get("stop_reason") or payload.get("termination_reason"))
        if not stop_reason:
    ...
    reference = _mapping(payload.get("complete_reference"))
    if not reference:
        reference = _mapping(_node_artifact(payload, "O05").get("complete_reference"))
    draft = _mapping(payload.get("final_draft"))
    if not draft:
    ...
    final_text = _clean(draft.get("text") or payload.get("final_text"))
    ...
    final_title = _clean(draft.get("title"))
