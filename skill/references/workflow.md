...
flowchart TD
    ...
    T3 --> G1[G01 凿agu适用性判断]
    G1 -->|SUITABLE_STRICT| G2[G02-G03 严格回填与审计]
    G1 -->|CONTROLLED_REWRITE_CANDIDATE| C1[C01 受控改写计划]
    G1 -->|NOT_SUITABLE| X[停止或等待人工干预]
    C1 --> C2[C02 受控候选生成]
    C2 --> C3[C03 改动预算与辨识度审计]
    C3 -->|PASS| G4[G04-G08 评分、选择、草稿与准入建议]
    C3 -->|FAIL| X
    G2 -->|PASS| G4
    G2 -->|FAIL| X
    L --> G6[G06-G08 草稿、查重与准入建议]
    G4 --> H[H 等待人工评价、准入和策略更新]
    G6 --> H
    H -->|持续执行且评价已提交| A
```
...
| V05.5 | 程序聚合精简证据并计算真实差异覆盖；主管标注生成高价值位置 | 精简包、差异跨度、覆盖缺口；不提前生成模板 |
| V06 | AUTO 按结构和高价值差异覆盖路由；显式 STRICT_SLOT 判断启动充分性 | 路由、定向 micro-worker、补搜或放弃 |
...
| G01 | 判断现有槽位能否严格回填；否则判断是否只需一次共指局部联动 | SUITABLE_STRICT / CONTROLLED_REWRITE_CANDIDATE / NOT_SUITABLE |
| G02/G03 | 生成并硬审计候选集 | 至少一条最小替换、全部可按模板重建 |
| C01 | 规划唯一实体槽替换和唯一共指局部谓词改写 | 共指链、许可范围、操作类型、施受角色、保留锚点 |
| C02 | 在改动预算内生成受控候选 | 至少一条最小联动型，不新增第二笑点 |
| C03 | 逐字审计改动预算、施受关系和原梗辨识度 | PASS / FAIL 及逐条淘汰理由 |
...
- 可用隔离 CLI 时，边界/变式搜索由不加载完整 Skill 的短生命周期 worker 执行，主管只接收程序化精简包；搜索事实仍须反向追到 worker 工具记录。
...
G01=SUITABLE_STRICT          → G02，generation_mode=STRICT_FILL
G01=CONTROLLED_REWRITE_CANDIDATE → C01，generation_mode=CONTROLLED_REWRITE；禁止 G02
G01=NOT_SUITABLE             → 停止生成，禁止 G02/C01
G03 != PASS                  → 修订候选或停止，禁止 G04
C03=PASS                     → G04；沿用受控候选及其改写计划
C03=FAIL                     → H01 或停止；禁止增加预算、禁止转入 LONG_FORM、禁止 G04
