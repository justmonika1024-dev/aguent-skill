...
只读取实际呈现的自然搜索结果。广告、AI 汇总、相关搜索词和搜索引擎自动补全文本不得伪装成来源正文。搜索摘要可用于发现候选和下一条查询的真实异写，但默认不能单独成为严格变式证据。
...
- O04、V02/V03、T04 的浏览器搜索与抓取结果标记 `TOOL_EXECUTED`，并在 `tool_call_refs` 中引用上述 `call_id`。
- O03、V01、T03 仍由主管规划，标记 `SUPERVISOR_REASONED`；浏览器只负责执行，不负责替状态机决定搜什么。
- V04/V05 只从 `SOURCE_FETCH.content_blocks` 抽取与验收严格变式。搜索摘要最多作为 `UNCERTAIN` 线索。
...
一轮使用浏览器搜索后，交付物必须能从每条接受的严格变式反向追到：变式原文 → 正文块 → 来源页 → 搜索结果 → 查询及推导理由。缺任一环，不能称为真实全链路完成。
